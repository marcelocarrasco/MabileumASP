...
CREATE OR REPLACE PACKAGE BODY G_MOBILEUM_ASP AS
  ...
  PROCEDURE P_MOBILEUM_ASP_DAY(P_FECHA IN VARCHAR2) AS
    ...
    SELECT  PAIS,
            trunc(FECHA)                                      FECHA,
            round(NVL(SUM(CNT_LLAMADAS),0),2)                 CNT_LLAMADAS,
            round(NVL(SUM(CNT_POLITICA_RESTRICCION),0),2)     CNT_POLITICA_RESTRICCION,
            round(NVL(SUM(CNT_BOLQ_REL_TIPO_GEST),0),2)       CNT_BOLQ_REL_TIPO_GEST,
            round(NVL(SUM(CNT_BLOQ_RAN_REL),0),2)             CNT_BLOQ_RAN_REL,
            round(NVL(SUM(CNT_PERMITIERON_CONTINUAR),0),2)    CNT_PERMITIERON_CONTINUAR,
            round(NVL(SUM(CNT_CONECTADAS_VOICE_MAIL),0),2)    CNT_CONECTADAS_VOICE_MAIL,
            round(NVL(SUM(CNT_CONECTADA_ANUNCIO_SWITCH),0),2) CNT_CONECTADA_ANUNCIO_SWITCH
